...
module.exports = {
  ...
  run: async(client, interaction) => {
	...
	const destek = new Discord.ButtonBuilder().setLabel('Destek Sunucum').setStyle('Link').setEmoji('1044325557615202364').setURL('https://discord.gg/rtTwrAKUaT');
    const row = new Discord.ActionRowBuilder().addComponents(dvt).addComponents(destek)
    const embed = new EmbedBuilder()
    .setAuthor({ name: "Merhaba, Ben Aries!", iconURL: interaction.client.user.displayAvatarURL({ dynamic: true })})
.setTitle("Aries'i Davet Et!")
.setDescription(`<:Bot:1106994122344104067> | Aries'i şimdi sunucuna davet et ve botun tadını çıkar!`)
.setColor('#2F3136')
.setTimestamp()

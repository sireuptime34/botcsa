    ...
    switch(reklamEngelSystemTrue) {
      case "ac": {
        ...
        if (reklamEngelSystem && reklamEngelSystemDate) {
            ...
            .setDescription(`<:carpi:1117083838590492772> | Bu sistem <t:${parseInt(reklamEngelSystemDate.date / 1000)}:R> önce açılmış!`)
        ...
        return interaction.reply({ content: "<:tik:1117083842264703016> | Başarılı bir şekilde sistem açıldı!" });
      }
  
      case "kapat": {
        if(!reklamEngelSystem) return interaction.reply({ content: "<:carpi:1117083838590492772> | Bu sistem zaten kapalı?" });
  
        db.delete(`reklamengel_${interaction.guild.id}`)
		db.delete(`reklamengelDate_${interaction.guild.id}`)
        return interaction.reply({ content: "<:tik:1117083842264703016> | Başarılı bir şekilde sistem kapatıldı!" });
      }
    }
